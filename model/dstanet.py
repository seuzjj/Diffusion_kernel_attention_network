...
class STKernelAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=3, num_node=25, num_frame=32,
                 kernel_size=1, stride=1, glo_reg_s=True, att_s=True, glo_reg_t=True, att_t=True,
                 use_temporal_att=True, use_spatial_att=True, attentiondrop=0, use_pes=True, use_pet=True, diffusion=0):
        ...
        if self.use_spatial_att:
            ...
            if self.att_s:
                ...
                dism = torch.cdist(y.view(y.shape[0], y.shape[1], y.shape[2]*y.shape[3]),
                                   y.view(y.shape[0], y.shape[1], y.shape[2]*y.shape[3]), p=2)
...
class DSTANet(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=32, num_subset=3, dropout=0., config=None, num_person=2,
                 ...
                 use_temporal_att=True, use_spatial_att=True, attentiondrop=0, dropout2d=0, use_pet=True, use_pes=True, kernelattention=True, diffusion=0):
        ...
        for index, (in_channels, out_channels, inter_channels, stride) in enumerate(config):
            if not kernelattention:# use original linear attention
                ...
                self.graph_layers.append(
                    STKernelAttentionBlock(in_channels, out_channels, inter_channels, stride=stride, num_frame=num_frame,
                                     **param))
